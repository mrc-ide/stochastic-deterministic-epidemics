combined_onward_restart <- function(onward, restart_date, 
 ...
 f <- function(x){
    ret <- x$state$state[, , which(x$state$time == sircovid_restart_date)]
    info_epoch_2 <- x$info$info[[2]]
    info_epoch_3 <- x$info$info[[3]]
    ret <- sircovid::inflate_state_strains(ret, info_epoch_2, info_epoch_3)
    ret
  }
  
 
  onward$state <- lapply(restart, f
                         )
